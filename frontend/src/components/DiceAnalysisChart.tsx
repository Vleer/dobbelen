        ...
        {[1, 2, 3, 4, 5, 6].map(faceValue => {
          ...
          return (
            <div key={faceValue} className="flex flex-col items-center">
              <div className="text-xs text-gray-700 mb-2 font-bold">{faceValue}</div>
                  ...
                  {allDiceOfValue.map(({ player, dice }, playerIndex) => 
                    dice.map((_, diceIndex) => (
                      <div
                        ...
                        style={{ borderColor: getPlayerHexColor(game.players.indexOf(player)), borderStyle: 'solid', backgroundColor: '#fff' }}
                      >
                        <div className="w-6 h-6 rounded-full border-4 border-black flex items-center justify-center" style={{ backgroundColor: getPlayerHexColor(game.players.indexOf(player)) }}>
              ...
              <div className="text-xs text-gray-600 mt-1 font-bold">{totalCount}</div>
            </div>
          );
        })}
      </div>
