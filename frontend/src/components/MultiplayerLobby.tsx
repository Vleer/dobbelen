...
// Smart AI names (subset focused on strategic-sounding names)
const SMART_AI_NAMES = [
  'Julia', 'Eva', 'Nina', 'Lisa', 'Diana', 'Anna', 'Rosa', 'Iris',
  'Vincent', 'Maarten', 'Gerard', 'Otto', 'Paul', 'Hendrik', 'Kees'
];
...
const MultiplayerLobby: React.FC<MultiplayerLobbyProps> = ({ onGameStart, onBack }) => {
  ...
  // Auto-focus the player name input when component initializes
  useEffect(() => {
    if (isInitialized && playerName && playerNameInputRef.current) {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        // Register AI players when game is updated (ensure they stay registered through round transitions)
  ...
  const addAIPlayer = async () => {
    ...
    try {
      ...
      // Immediately register the basic AI player
      const newPlayer = updatedGame.players.find(p => p.name === aiName);
      if (newPlayer) {
        aiService.registerAIPlayer(newPlayer.id, newPlayer.name, 'basic');
      }
  ...
  const removeAIPlayer = async (playerId: string) => {
    ...
    try {
      // Only allow removing AI players (both basic and smart)
      const player = game.players.find(p => p.id === playerId);
      if (!player || (!player.name.startsWith('AI ') && !player.name.startsWith('Smart AI '))) {
        console.warn('Cannot remove non-AI player');
        return;
      }
      
      // Call the backend to actually remove the player
      ...
      setError(t('errors.failedToRemovePlayer') || 'Failed to remove AI player');
    }
  };

  const addSmartAIPlayer = async () => {
    if (!game || !isHost) return;
    
    try {
      // Get all existing Smart AI player names to avoid duplicates
      const existingSmartAINames = game.players
        .filter(p => p.name.startsWith('Smart AI '))
        .map(p => p.name.replace('Smart AI ', ''));
      
      let smartAIName;
      do {
        const randomName = SMART_AI_NAMES[Math.floor(Math.random() * SMART_AI_NAMES.length)];
        smartAIName = `Smart AI ${randomName}`;
      } while (existingSmartAINames.includes(smartAIName.replace('Smart AI ', '')));
      
      const updatedGame = await gameApi.joinMultiplayerGame(game.id, smartAIName);
      setGame(updatedGame);
      
      // Immediately register the Smart AI player
      const newPlayer = updatedGame.players.find(p => p.name === smartAIName);
      if (newPlayer) {
        aiService.registerAIPlayer(newPlayer.id, newPlayer.name, 'smart');
      }
      
      console.log('Added Smart AI player:', smartAIName);
    } catch (err: any) {
      console.error('Error adding Smart AI player:', err);
      setError(t('errors.failedToAddPlayer'));
    }
  };
              ...
              <div>
                <div className="flex space-x-2">
                      <input
                        ...
                        maxLength={3}
                        ref={gameIdInputRef}
                        onClick={() => gameIdInputRef.current?.select()}
                      />
                  <button
                    onClick={joinGame}
          ...
          <div className="space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {game.players.map((player, index) => (
                  <div key={player.id} className="flex items-center bg-white p-2 rounded">
                    ...
                    {isHost && (player.name.startsWith('AI ') || player.name.startsWith('Smart AI ')) && (
                      <button
                        ...
                        title={`Remove ${player.name.startsWith('Smart AI ') ? 'Smart AI' : 'AI'} player`}
                      >
                        ×
                      </button>
                    )}
            ...
            {isHost && (
              <div className="space-y-2 md:space-y-3">
                ...
                <button
                  onClick={addSmartAIPlayer}
                  className="w-full py-2 md:py-3 px-3 md:px-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-bold text-sm md:text-base"
                  disabled={game.players.length >= 6}
                >
                  🧠 Add Smart AI ({game.players.length}/6)
                </button>
                <button
                  onClick={async () => {
